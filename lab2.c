...
int** read_board_from_file(char* filename){
    ...
    //Open The File
    fp = fopen(filename, "r");
    //If File cannot be open, send error and return a Null Board
    if (fp == NULL){
    ...
    //Assign board csv values to board
    for(int row = 0; row < ROW_SIZE; row++){
    for(int col = 0; col < COL_SIZE; col++){
            // number from the board at the row and column   
            int num;
            ...
            if (status != 1){
                    //If any errors from fscanf, throw error, free board memory, close file and return null board
            ...
            //assign board at the row and column from the file
            board[row][col] = num;
    }
}       
...
int is_board_valid(){
    ...
    //allocates memory for parameter array
...
void *runner(void *param){
    ...
    //dereference params and assign them to local variables in the thread's stack
    int id = params->id;
    ...
    //initially assume this thread's section of the board is valid
    int isThreadValid = 1;
    //Create Array to both see if any duplicates, and see if the section has 1-9
    int check[9] = {0};
    ...
    //if the thread's section of the board is invalid, set 0 to worker validation index
    if (!isThreadValid){
